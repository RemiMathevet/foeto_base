...
Tables : entites_livres (entite_id, nom, orpha, n_titres, livres) et
...
def main():
    ...
    c.execute("create table entites_livres (entite_id text primary key, nom text, orpha text, n_titres integer, livres text)")
    ...
    for key, ts in groupes.items():
        ...
        c.execute("insert into entites_livres values(?,?,?,?,?)", (key, nom, key if key.startswith("ORPHA:") else None, len(titres), ",".join(livres)))
    ...
    print(f"{len(rows)} entrées de livres -> {n} entités ; {multi} décrites par plusieurs livres ; "
          f"{sans} sans ORPHA dont {fus_nom} fusionnées par le nom")
