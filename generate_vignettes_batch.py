#!/usr/bin/env python3
"""Génération batch de vignettes cliniques fœtopathologiques via Anthropic Batch API.

Pour chaque syndrome de la DB ayant assez de données :
1. Construit un prompt avec nom, HPO, signes prénataux, discriminateurs
2. Demande à Sonnet de générer 2 vignettes cliniques fœtopathologiques
3. Soumet le tout via l'API Batch (asynchrone, -50% coût)

Usage:
    python generate_vignettes_batch.py submit          # soumettre le batch
    python generate_vignettes_batch.py status           # vérifier le statut
    python generate_vignettes_batch.py collect           # collecter les résultats
    python generate_vignettes_batch.py submit --limit 10 # test sur 10
"""
import argparse
import json
import sqlite3
...
RESULTS_FILE = OUT_DIR / "vignettes_results.jsonl"
MODEL = "claude-sonnet-4-6"

SYSTEM_PROMPT = """Tu es un fœtopathologiste expérimenté qui rédige des vignettes cliniques pour un benchmark de diagnostic.

Pour chaque syndrome, génère exactement 2 vignettes cliniques DISTINCTES en français, au format JSON.

Chaque vignette doit :
- Simuler un cas fœtopathologique réaliste (IMG, MFIU, ou naissance avec décès néonatal)
...
- Varier la sévérité et la présentation entre les 2 vignettes
- Faire 150-250 mots chacune
- Utiliser le vocabulaire fœtopathologique français standard

Format de sortie STRICT (JSON) :
{"vignettes": [{"text": "...", "severity": "classique|sévère|modéré|atypique"}, {"text": "...", "severity": "..."}]}"""


def build_syndrome_prompt(row):
    """Build the user prompt for a syndrome."""
    parts = []
    ...
    if row['hpo_labels']:
        hpo_list = row['hpo_labels'][:1000]
        parts.append(f"\nTermes HPO principaux :\n{hpo_list}")

    parts.append("\nGénère 2 vignettes cliniques fœtopathologiques distinctes pour ce syndrome.")
    return "\n".join(parts)


def load_syndromes(conn, limit=None):
    """Load syndromes with enough data for vignette generation."""
    query = """
...
def cmd_submit(args):
    ...
    print(f"Building batch for {len(syndromes)} syndromes...")
    ...
    # Estimate tokens
    total_chars = sum(len(SYSTEM_PROMPT) + len(build_syndrome_prompt(s)) for s in syndromes)
    ...
    with open(OUT_DIR / "batch_meta.json", "w") as f:
...
def cmd_status(args):
    ...
    if total > 0:
        print(f"Progress: {counts.succeeded}/{total} ({counts.succeeded/total*100:.1f}%)")
...
def cmd_collect(args):
    ...
    for result in client.messages.batches.results(batch_id):
        syndrome_id = result.custom_id.replace("_", ":", 1)  # ORPHA_123 → ORPHA:123
    ...
    if n_ok > 0:
        ...
        for r in results:
            ...
            for i, v in enumerate(r["vignettes"]):
                text = v.get("text", "")
                severity = v.get("severity", "classique")
                if len(text) < 50:
                    continue
                try:
                    conn.execute(
                        "INSERT OR REPLACE INTO synthetic_vignettes (syndrome_id, vignette_index, clinical_text, severity) "
                        "VALUES (?, ?, ?, ?)",
                        (r["syndrome_id"], i, text, severity)
                    )
                    n_inserted += 1
        ...
        conn.commit()
        conn.close()
        print(f"  Inserted {n_inserted} vignettes into synthetic_vignettes table")
